                        ...
                        @foreach ($days as $day)
                            <td class="border border-gray-300 p-2">
                                ...
                                @if ($currentJadwal->isNotEmpty())
                                    ...
                                    <div class="w-40 h-auto p-2 bg-white border-l-4 border-blue-500 rounded-lg shadow-lg mb-2 jadwal-item border-yellow-300
    @if(in_array($item->id_jadwal, $selectedJadwals)) border-green-500 bg-green-400 @endif"
...
<script>
    $(document).ready(function() {
        ...
        var days = @json($days);
        console.log(timeslots); // Cek di console jika data berhasil dikirim ke JS
        console.log(days); 
        var selectedJadwals = @json($selectedJadwal);
        var totalSks = 0;
        var maxSks = {{ $maxSKS }};
        ...
        function markConflictingSchedules() {
            selectedJadwals.forEach(function(jadwalId) {
                var jadwalItem = $(`[data-jadwal-id="${jadwalId}"]`);
                var hari = jadwalItem.data('hari');
                var start1 = convertTimeToMinutes(jadwalItem.data('jam-mulai'));
                var end1 = convertTimeToMinutes(jadwalItem.data('jam-selesai'));

                $('.jadwal-item').not(`[data-jadwal-id="${jadwalId}"]`).each(function() {
                    if ($(this).data('hari') === hari) {
                        var start2 = convertTimeToMinutes($(this).data('jam-mulai'));
                        var end2 = convertTimeToMinutes($(this).data('jam-selesai'));
    ...
    // Fungsi untuk mengonversi waktu ke total menit
    function convertTimeToMinutes(time) {
            ...
            if (isChecked) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        data.forEach(function(jadwal) {
                            addJadwalToTable(jadwal);
                            selectedJadwals.push(jadwal.id_jadwal);
                        });
                        markConflictingSchedules();
        ...
        function addJadwalToTable(jadwal) {
            ...
            // Flatten the nested data from matakuliah and ruang if they exist
            ...
            if (jadwal.ruang) {
                ...
                // Combine gedung and nama into a single ruangan field
            ...
            let jamMulaiFormatted = jadwal.jam_mulai.substring(0, 5); // e.g. '07:00:00' -> '07:00'
            // If it's like '7:00', pad it to '07:00'
            ...
            // If you need to round to the nearest hour, uncomment the next line:
            // jamMulaiFormatted = roundToNearestHour(jamMulaiFormatted);
            ...
            if (row.length > 0) {
                ...
                const jadwalDiv = $(`
                    <div class="w-full p-2 bg-white border-l-4 rounded-lg shadow-lg mb-2 jadwal-item border-yellow-300" data-jadwal-id="${jadwal.id_jadwal}" data-kode-mk="${jadwal.kode_mk}" data-nama-mk="${jadwal.nama_mk}">
                        <div>
       ...
       // Fungsi untuk menghapus jadwal dari tabel
        ...
        // Fungsi untuk memilih jadwal dan memasukkan ke IRS melalui konfirmasi SweetAlert2
        // Misalnya ketika pengguna menekan tombol "Pilih Jadwal"
$('.pilih-jadwal').on('click', function() {
    var jadwalId = $(this).data('jadwal-id');
    
    $.ajax({
        url: '/pilih-jadwal',  // Ganti dengan URL yang sesuai
        method: 'POST',
        data: {
            jadwal_id: jadwalId,
        ...
        success: function(response) {
            if (response.status === 'success') {
                ...
                // Update UI, beri warna hijau
                $(`[data-jadwal-id="${jadwalId}"]`).addClass('border-green-500 bg-green-100');
            } else {
