...
class BuatIRSController extends Controller
{
    ...
    public function index()
    {
        ...
        ->orWhereIn('kode_mk', $selectedMataKuliah) // Sertakan mata kuliah yang sudah dipilih, meskipun di luar semester
        ->orderBy('semester', 'asc')
        ->get();
        // Ambil semua jadwal untuk semester saat ini atau yang sudah dipilih
        $jadwal = Jadwal::whereHas('matakuliah', function($query) use ($semester) {
            $query->where('semester', $semester);
        })
        ->orWhereIn('id_jadwal', $selectedJadwal)
        ->with(['waktu', 'matakuliah', 'ruang'])
        ->get()
        ->map(function ($item) use ($semester) {
            // Tandai jadwal yang sesuai dengan semester aktif
        ...
        return view('content.mhs.akademik', compact('mhs', 'status', 'semester', 'mataKuliah', 'ipk', 'ips', 'maxSKS', 'conflictingJadwal', 'jadwal', 'selectedJadwal', 'selectedMataKuliah', 'timeslots', 'days'));
    }
