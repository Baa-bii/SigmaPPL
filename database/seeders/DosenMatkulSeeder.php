<?php

namespace Database\Seeders;

use Illuminate\Database\Console\Seeds\WithoutModelEvents;
use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;
use App\Models\DosenMatkul;

class DosenMatkulSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        DosenMatkul::create([
            'nip_dosen' => '1982030920060410',
            'kode_mk' => 'PAIK6102',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1989030320150420',
            'kode_mk' => 'PAIK6102',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1958090119860320',
            'kode_mk' => 'PAIK6101',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1985063020121210',
            'kode_mk' => 'PAIK6101',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1979072020031210',
            'kode_mk' => 'PAIK6105',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1996030320220410',
            'kode_mk' => 'PAIK6105',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1971081119970210',
            'kode_mk' => 'PAIK6105',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1985112520180320',
            'kode_mk' => 'PAIK6103',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1981062020150410',
            'kode_mk' => 'PAIK6103',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1979052420091210',
            'kode_mk' => 'PAIK6104',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1971081119970210',
            'kode_mk' => 'PAIK6104',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1974040119990310',
            'kode_mk' => 'PAIK6202',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1965110719920310',
            'kode_mk' => 'PAIK6202',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1988061420221020',
            'kode_mk' => 'PAIK6603',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1979072020031210',
            'kode_mk' => 'PAIK6603',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1989030320150420',
            'kode_mk' => 'PAIK6603',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1981062020150410',
            'kode_mk' => 'PAIK6603',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' => '1981042020050120',
            'kode_mk' => 'PAIK6204',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1970070519970210',
            'kode_mk' => 'PAIK6204',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1971081119970210',
            'kode_mk' => 'PAIK6204',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1985112520180320',
            'kode_mk' => 'PAIK6203',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1981062020150410',
            'kode_mk' => 'PAIK6203',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1988032220201210',
            'kode_mk' => 'PAIK6402',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1982030920060410',
            'kode_mk' => 'PAIK6402',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1979052420091210',
            'kode_mk' => 'PAIK6402',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1980122720150410',
            'kode_mk' => 'PAIK6402',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1988061420221020',
            'kode_mk' => 'PAIK6402',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1971081119970210',
            'kode_mk' => 'PAIK6402',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1985063020121210',
            'kode_mk' => 'PAIK6201',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1973122020001210',
            'kode_mk' => 'PAIK6201',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1970070519970210',
            'kode_mk' => 'UUW00006',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1988061420221020',
            'kode_mk' => 'UUW00007',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1972112119980210',
            'kode_mk' => 'UUW00005',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1979072020031210',
            'kode_mk' => 'UUW00003',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1979021220081210',
            'kode_mk' => 'UUW00003',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1965110719920310',
            'kode_mk' => 'UUW00011',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1978051620031210',
            'kode_mk' => 'UUW00011',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1980091420060410',
            'kode_mk' => 'UUW00004',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1981042020050120',
            'kode_mk' => 'UUW00004',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1980122720150410',
            'kode_mk' => 'UUW00006',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1988061420221020',
            'kode_mk' => 'UUW00006',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1965110719920310',
            'kode_mk' => 'UUW00006',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1980091420060410',
            'kode_mk' => 'PAIK6301',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1996030320220410',
            'kode_mk' => 'PAIK6301',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1973082919980220',
            'kode_mk' => 'PAIK6303',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1983020320060410',
            'kode_mk' => 'PAIK6303',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1992042520230720',
            'kode_mk' => 'PAIK6306',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1961092819860320',
            'kode_mk' => 'PAIK6306',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1980122720150410',
            'kode_mk' => 'PAIK6302',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1965110719920310',
            'kode_mk' => 'PAIK6302',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    
        DosenMatkul::create([
            'nip_dosen' =>'1980091420060410',
            'kode_mk' => 'PAIK6506',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1988032220201210',
            'kode_mk' => 'PAIK6506',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1988061420221020',
            'kode_mk' => 'PAIK6506',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1970070519970210',
            'kode_mk' => 'PAIK6304',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1979021220081210',
            'kode_mk' => 'PAIK6304',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1996022120230720',
            'kode_mk' => 'PAIK6304',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1965110719920310',
            'kode_mk' => 'PAIK6305',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1976011020091220',
            'kode_mk' => 'PAIK6305',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1988032220201210',
            'kode_mk' => 'UUW00008',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1983020320060410',
            'kode_mk' => 'UUW00008',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1974040119990310',
            'kode_mk' => 'UUW00008',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1978051620031210',
            'kode_mk' => 'PAIK6406',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1996030320220410',
            'kode_mk' => 'PAIK6406',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1971081119970210',
            'kode_mk' => 'PAIK6404',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1978051620031210',
            'kode_mk' => 'PAIK6404',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1980091420060410',
            'kode_mk' => 'PAIK6401',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1989030320150420',
            'kode_mk' => 'PAIK6401',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1980091420060410',
            'kode_mk' => 'PAIK6403',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1973082919980220',
            'kode_mk' => 'PAIK6403',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1982030920060410',
            'kode_mk' => 'PAIK6601',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1996030320220410',
            'kode_mk' => 'PAIK6601',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1974040119990310',
            'kode_mk' => 'PAIK6405',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        DosenMatkul::create([
            'nip_dosen' =>'1983020320060410',
            'kode_mk' => 'PAIK6405',
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    }
}
