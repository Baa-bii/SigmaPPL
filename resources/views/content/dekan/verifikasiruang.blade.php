                ...
                <div class="relative inline-block text-left">
                    ...
                    <div id="dropdownFilters" class="z-10 hidden border border-gray-300 bg-white divide-y divide-gray-100 rounded-lg shadow w-50 dark:bg-gray-700 dark:divide-gray-600">
                        <ul class="py-1 text-xs text-gray-700 dark:text-gray-200 whitespace-nowrap" aria-labelledby="dropdownFiltersButton">
                            <li><a href="{{ route('dekan.ruang.filter', ['filter' => 'disetujui']) }}" class="block px-4 py-2 hover:bg-gray-600 hover:text-white dark:hover:bg-gray-600 dark:hover:text-white {{ request('filter') == 'disetujui' ? 'bg-gray-300' : '' }}">Semua Sudah Disetujui</a></li>
                            <li><a href="{{ route('dekan.ruang.filter', ['filter' => 'ditolak']) }}" class="block px-4 py-2 hover:bg-gray-600 hover:text-white dark:hover:bg-gray-600 dark:hover:text-white {{ request('filter') == 'ditolak' ? 'bg-gray-300' : '' }}">Semua Sudah Ditolak</a></li>
                            <li><a href="{{ route('dekan.ruang.filter', ['filter' => 'menunggu']) }}" class="block px-4 py-2 hover:bg-gray-600 hover:text-white dark:hover:bg-gray-600 dark:hover:text-white {{ request('filter') == 'menunggu' ? 'bg-gray-300' : '' }}">Semua Belum Diisi</a></li>
                        </ul>
                    </div>
                </div>
        ...
        <div class="overflow-x-auto mt-2">
            ...
            <table id="table-container" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tr class="bg-white text-black dark:bg-gray-800 status-row" data-status="{{ $data->status ?? 'menunggu' }}"
                    ...
                    <!-- Kelas -->
                    <td class="p-4 whitespace-nowrap text-xs text-center">{{ $data->jadwal->first()?->kelas ?? 'N/A' }}</td>
                    ...
                    <!-- Waktu -->
                    <td class="p-4 whitespace-nowrap text-xs">{{ $data->jadwal->first()?->waktu?->jam_mulai ?? 'N/A' }} - {{ $data->jadwal->first()?->waktu?->jam_selesai ?? 'N/A' }}</td>
